    ...
    get_multiplier = function() {
      ...
      multiplier = (function() {
        switch (false) {
          ...
          case !(amount >= 3000 && amount < 7000):
            return multiplier = 1.2;
        }
      })();
      return multiplier;
    };
    $('.size_input').click(function(event) {
      var array, current_value, eventPrices, multiplier, selected_value, sum;
      ...
      $('input[name=addservices]:checked').each(function() {
        var amount, value;
        value = $(this).val();
        amount = parseInt(value.split(" ")[1], 10);
        return eventPrices.push(amount);
      });
      if (eventPrices.length > 0) {
        sum = eventPrices.reduce((function(_this) {
          return function(a, b) {
            return a + b;
          };
        })(this));
      ...
      return $('#priceValue').text(sum + array * multiplier);
    });
    $('.addservice_checkbox').change(function(event) {
      var amount, array, current_value;
      amount = $(this).val();
      ...
      if (this.checked) {
        return $('#priceValue').text(current_value + array);
      } else {
        return $('#priceValue').text(current_value - array);
      }
    });
    return $('#amount').on('input', function(event) {
      var amount, array, eventPrices, multiplier, selected_value, sum;
      amount = $(this).val();
      multiplier = get_multiplier();
      selected_value = $('input[name=volume]:checked').val();
      array = selected_value.split(",")[1].replace(']', '');
